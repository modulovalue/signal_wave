...
/// Lets you test whether streams emit their values and events in an order relative to each other.
///
/// If you don't need typed keys then use [expectStreamsInOrderInline]
void expectStreamsInOrder(Iterable<StreamKey> keys, dynamic matcher, {Function(StreamEventType) onEach}) {
    final Map<dynamic, StreamSubscription> subs = <dynamic, StreamSubscription>{};

    assert(keys
        .map<dynamic>((a) => a.key)
        .toSet()
        .length == keys.length, "All keys need to be unique");

    final StreamController<StreamEventType> s = StreamController.broadcast(sync: true);
...
/// Inspired by TypedRegistry at https://github.com/google/charts/blob/35aeffe7c96aa7d231c90fddd9766998545f1080/charts_common/lib/src/common/typed_registry.dart
...
void expectStreamsInOrderInline(List<Stream> streams, dynamic Function(StreamKey Function(dynamic) k) matcher) {
...
FutureOr<T> subscribeDuring<T>(StreamSubscription<T> sub, FutureOr<T> Function() execute) async {
    final a = await execute();
    await sub.cancel();
    return a;
}

StreamSubscriptionDecorator<T> expectSub<T>(Stream<T> stream, dynamic matcher) => SurrogateStream<T>(matcher).listen(stream);
