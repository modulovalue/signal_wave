    ...
    group("other.dart", () {
        ...
        group("oneAsFuture", () {
            test("will immediately finish because signals have a value", () async {
                final wave = Wave.fromIterable([1, 2]);
                expect(wave.oneAsFuture().asStream(), emitsInOrder(<dynamic>[
                    const NextEvent<int>(1),
                    emitsDone,
                ]));
            });
        ...
        group("nextAsFuture", () {
            test("will finish on next value", () async {
                // ignore: close_sinks
                final wave = Wave.fromIterable([1, 2]);
                expect(wave.nextAsFuture().asStream(), emitsInOrder(<dynamic>[
                    const NextEvent<int>(2),
                    emitsDone,
                ]));
            });
        ...
        group("asStream", () {
            test("emits done when wave is done", () {
                expect(Wave.fromIterable([1, 2, 3]).asStream(), emitsInOrder(<dynamic>[1, 2, 3, emitsDone]));
            });
            ...
            test("signal behaves like a regular stream when first element is skipped", () async {
                final Signal<int> signal = Signal(1);
                final List<Object> data = [];
                // ignore: cancel_subscriptions
                signal.wave.skip(1).asStream().listen(data.add, onDone: () => data.add("done"));
                signal.add(2);
                signal.close();
                await pumpEventQueue();
