  ...
  group("operators.dart", () {
    ...
    group("flatMap", () {
      test("a:C b:NC", () {
        ...
        final Disposable _ =
            a.wave.flatMap((a) => b.wave).subscribeEvent(events.add);
        a.close();
        expect(events, [
          const NextEvent(2),
        ]);
      ...
      test("flatMap behavior like rxjs", () {
        ...
        final Wave<int> flatmapped =
            awave.flatMap<int>((i) => (i == 1) ? bwave : cwave);
        ...
        flatmapped
            .subscribeSink(EventSink((a) => events.add(KeyedEvent("fm", a))));
    ...
    group("switchMap", () {
      test("a:C b:NC", () {
        ...
        final Signal<int> b = Signal(2);
        final Disposable _ =
            a.wave.switchMap((a) => b.wave).subscribeEvent(events.add);
        a.close();
        expect(events, [
          const NextEvent(2),
        ]);
      ...
      test("switchMap behavior like rxjs", () {
        ...
        final Wave<int> flatmapped =
            awave.switchMap<int>((i) => (i == 1) ? bwave : cwave);
...
//                    const KeyedEvent("fm", NextEvent(30)),
      ...
      test(
          "cancelling a switchmap does remove all subscribers from the source waves",
          () {
        // ignore: close_sinks
        final Signal<int> a = Signal(1);
        // ignore: close_sinks
    ...
    test("where", () {
      expect(
          Wave.fromIterable([1, 2, 3]).where((a) => a != 2).asStream(),
    ...
    test("distinct", () {
      expect(
          Wave.fromIterable(["a", "b", "b", "c", "c", "c", "d"])
              .distinct()
              .asStream(),
          emitsInOrder(<dynamic>[
            "a",
      ...
      expect(
          Wave.fromIterable(["a", "a"]).distinct().asStream(),
          emitsInOrder(<dynamic>[
            "a",
            emitsDone,
          ]));
