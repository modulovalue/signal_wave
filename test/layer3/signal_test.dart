...
void main() {
    group("signal.dart", () {
        ...
        test("not subscribed waves keep their initial value which isn't updated if they are not subscribed to", () {
            final sut = Signal<int>(1);
            final wave = sut.wave;
            sut.add(2);
            expect(sut.value, 2);
        ...
        test("hasSubscribers is true inside the onListen method", () {
            ...
            Signal<int> sut;
            sut = Signal<int>(2, onListen: () => hasSubscribers.add(sut.hasSubscribers));
            expect(hasSubscribers, <dynamic>[]);
            sut.wave.subscribe();
            expect(hasSubscribers, <dynamic>[true]);
        });
        test("hasSubscribers is false inside the onCancel method when the last subscription was cancelled", () {
            final List<bool> hasSubscribers = [];
            Signal<int> sut;
            sut = Signal<int>(2, onCancel: () => hasSubscribers.add(sut.hasSubscribers));
            expect(hasSubscribers, <dynamic>[]);
        ...
        test("hasSubscribers is false inside the onCancel method when the signal was closed with listeners", () {
            final List<bool> hasSubscribers = [];
            Signal<int> sut;
            sut = Signal<int>(2, onCancel: () => hasSubscribers.add(sut.hasSubscribers));
            expect(hasSubscribers, <dynamic>[]);
        ...
        test("add operator", () {
            final sut = Signal<int>(1);
            final plusOne = sut.wave.as(IntWave.from) + 1;
            final plusTwo = sut.wave.as(IntWave.from) + 2;
        ...
        test("can emit values inside of onListen", () {
            ...
            signal = Signal<Option<int>>(const None(), onListen: () {
                signal.add(const Some(2));
            });
            ...
            expect(events, [
                const NextEvent<Option<int>>(None()),
                const NextEvent<Option<int>>(Some(2)),
                const CloseEvent<Option<int>>(),
            ]);
        });
        test("a cancelled wave doesn't make the subscription receive a closed event", () async {
