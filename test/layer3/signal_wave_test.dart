...
import 'package:signal_wave/wave_test.dart';
...
void main() {
  group("signal_wave.dart", () {
    ...
    group("ctors/static creators", () {
      ...
      test("periodic", () async {
        expectSub(
            Wave.timer(const Duration(milliseconds: 100), (a) => a).asStream(),
            emitsInOrder(<dynamic>[
              0,
              1,
              emitsDone,
    ...
    group("custom tests", () {
      ...
      test(
          "a cancelled subscription doesn't receive events when there are other active subscriptions",
          () {
        ...
        expect(events1, <Event<int>>[
          const NextEvent(0),
          const NextEvent(1),
          const CloseEvent<int>()
        ]);
        expect(events2, <Event<int>>[
          const NextEvent(0),
          const NextEvent(1),
          const NextEvent(2),
          const CloseEvent<int>()
