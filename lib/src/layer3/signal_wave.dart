...
import 'package:signal_wave/src/layer2/creation.dart';
import 'package:signal_wave/src/layer2/operators.dart';
import 'package:signal_wave/src/layer2/other.dart';
...
class Wave<S> extends WaveType<S> with WaveOtherMixin<S>, WaveOperators<S> {
    ...
    Wave.custom(this._value, Disposable Function(EventSink<S> sink) _subscribeHandler)
    ...
    factory Wave.fromIterable(Iterable<S> collection) => fromIterableWave(collection);

    static Wave<Option<Either<dynamic, S>>> fromStream<S>(Stream<S> stream) => fromStreamWave(stream);

    factory Wave.generate(int times, S Function(int i) map) => generateWave(times, map);

    factory Wave.timer(Duration duration, S Function(int) valueComputation) => periodic<S>(duration, valueComputation);

    static Wave<void> updateEvery(Duration duration) => periodic<void>(duration, (a) => null);

    static Wave<int> updateEverySecond() => periodic<int>(const Duration(milliseconds: 1000), (a) => a);
    ...
    static Wave<T> combineLatest<S, T>(Iterable<WaveType<S>> list, T Function(Iterable<S>) map) {
        return combineLatestWave(list, map);
    }

    static Wave<R> combineLatest2<A, B, R>(WaveType<A> a, WaveType<B> b, R Function(A, B) map) {
        return combineLatestWave<dynamic, R>([a, b], (li) {
            return map(li.elementAt(0) as A, li.elementAt(1) as B);
        });
    }

    static Wave<R> combineLatest3<A, B, C, R>(WaveType<A> a, WaveType<B> b, WaveType<C> c, R Function(A, B, C) map) {
        return combineLatestWave<dynamic, R>([a, b, c], (li) {
            return map(li.elementAt(0) as A, li.elementAt(1) as B, li.elementAt(2) as C);
        });
    }

    static Wave<R> combineLatest4<A, B, C, D, R>(WaveType<A> a, WaveType<B> b, WaveType<C> c, WaveType<D> d,
        R Function(A, B, C, D) map) {
        return combineLatestWave<dynamic, R>([a, b, c, d], (li) {
            return map(li.elementAt(0) as A, li.elementAt(1) as B, li.elementAt(2) as C, li.elementAt(3) as D);
        });
    }

    static Wave<R> combineLatest5<A, B, C, D, E, R>(WaveType<A> a, WaveType<B> b, WaveType<C> c, WaveType<D> d, WaveType<E> e,
        R Function(A, B, C, D, E) map) {
        return combineLatestWave<dynamic, R>([a, b, c, d, e], (li) {
            return map(
                li.elementAt(0) as A, li.elementAt(1) as B, li.elementAt(2) as C, li.elementAt(3) as D, li.elementAt(4) as E);
        });
    }

    static Wave<R> combineLatest6<A, B, C, D, E, F, R>(WaveType<A> a, WaveType<B> b, WaveType<C> c, WaveType<D> d,
        WaveType<E> e, WaveType<F> f,
    ...
    static Wave<R> combineLatest7<A, B, C, D, E, F, G, R>(WaveType<A> a, WaveType<B> b, WaveType<C> c, WaveType<D> d,
        WaveType<E> e, WaveType<F> f,
        WaveType<G> g, R Function(A, B, C, D, E, F, G) map) {
    ...
    static Wave<R> combineLatest8<A, B, C, D, E, F, G, H, R>(WaveType<A> a, WaveType<B> b, WaveType<C> c, WaveType<D> d,
        WaveType<E> e, WaveType<F> f,
        WaveType<G> g, WaveType<H>h, R Function(A, B, C, D, E, F, G, H) map) {
    ...
    static Wave<R> combineLatest9<A, B, C, D, E, F, G, H, I, R>(WaveType<A> a, WaveType<B> b, WaveType<C> c, WaveType<D> d,
        WaveType<E> e, WaveType<F> f,
        WaveType<G> g, WaveType<H>h, WaveType<I> i, R Function(A, B, C, D, E, F, G, H, I) map) {
    ...
    static Wave<Iterable<MapEntry<T, O>>> mapWithWave<T, O>(Iterable<T> data, Wave<O> Function(T t) other) {
        return Wave.combineLatestID(data.map((a) {
            return other(a).map((O b) => MapEntry(a, b));
        }));
    }

    static Wave<Iterable<T>> whereWave<T>(Iterable<T> data, Wave<bool> Function(T t) filter) {
        return mapWithWave<T, bool>(data, filter).map((Iterable<MapEntry<T, bool>> a) {
            return a.where((a) => a.value).map((a) => a.key);
        });
    }

    static Wave<Iterable<T>> waveWaveWhere<T>(Wave<Iterable<T>> data, Wave<bool> Function(T t) filter) {
