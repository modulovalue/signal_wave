...
abstract class Signal<S> extends SignalType<S> {
  factory Signal(S _value,
      {void Function() onListen,
      void Function() onCancel,
      void Function(Signal<S>) onInit}) {
    return _Signal<S>(_value,
        onListen: onListen, onCancel: onCancel, onInit: onInit);
  }
...
class _Signal<S> extends Signal<S> {
  ...
  Wave<S> get _wave => Wave.custom(value, (sink) {
        if (_isClosed) return Disposable(sink.close);
        ...
        if (subscribersWereEmpty) _onListen();

        return disposable = Disposable(() {
          _subscribers.remove(realSink);
          realSink.close();
          if (_subscribers.isEmpty) _onCancel();
          realSink = null;
          disposable = null;
        });
      });
  ...
  void add(S data) {
    ...
    Set<EventSink<S>>.from(_subscribers).forEach((c) => c.add(data));
  }
  ...
  void close() {
    if (!_isClosed) {
      _isClosed = true;
      // to prevent Concurrent modification during iteration.
      Set<EventSink<S>>.from(_subscribers).forEach((c) => c.close());
    }
  }
}
