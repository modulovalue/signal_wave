...
Wave<Option<Either<dynamic, S>>> fromStreamWave<S>(Stream<S> stream) {
    return Wave.custom(const None(),
            (EventSink<Option<Either<dynamic, S>>> sink) {
            sink.add(const None());
            final sub = stream.listen(
                    (data) => sink.add(Some(Right<dynamic, S>(data))),
                onError: (dynamic err) {
...
Wave<S> periodic<S>(Duration duration, S Function(int) valueComputation) {
    ...
    return Wave.custom(initial, (EventSink<S> sink) {
        ...
        final subscription = Stream<S>.periodic(duration, (a) => valueComputation(a + 1)).listen(
                (a) {
